...
export default function ACLARRegistryPage() {
  ...
  async function openDrawer(id: string) {
    ...
    if (record) {
      ...
      setLegalNotesDraft(record.legalNotes ?? "")
      setDrawerOpen(true)
    }
  }
  function saveLegalNotes() {
    if (!selectedRecord) return
    setSelectedRecord({ ...selectedRecord, legalNotes: legalNotesDraft })
    toast.success("Legal notes saved")
  }
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="text-muted-foreground text-left border-b border-border/30">
                ...
                <th className="px-3 py-1.5 font-medium">Linked Contracts</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-3 py-12 text-center text-sm text-muted-foreground">No consent records match the current filters.</td></tr>
                ...
                >
                  ...
                  <td className="px-3 py-1">
                    {record.linkedContractIds && record.linkedContractIds.length > 0 ? (
                      <span className="flex flex-wrap gap-0.5">
                        {record.linkedContractIds.slice(0, 2).map((cid) => (
                          <Link key={cid} href={`/contracts/${cid}`} className="font-mono text-[10px] text-primary hover:underline" onClick={(e) => e.stopPropagation()}>
                            {cid}
                          </Link>
                        ))}
                        {record.linkedContractIds.length > 2 && <span className="text-muted-foreground text-[10px]">+{record.linkedContractIds.length - 2}</span>}
                      </span>
                    ) : (
        ...
        <SheetContent side="right" className="w-[400px] sm:w-[400px] p-0">
          ...
          {selectedRecord && (
            <div className="px-4 py-3 space-y-4 overflow-y-auto h-[calc(100vh-60px)]">
              ...
              {selectedRecord.contractCoverage && (
                <div>
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider mb-1">Contract Coverage</div>
                  <div className="text-[13px] font-medium">
                    {selectedRecord.contractCoverage.covered} of {selectedRecord.contractCoverage.total} active contracts covered by this consent
                  </div>
                </div>
              )}

              {selectedRecord.linkedContractIds && selectedRecord.linkedContractIds.length > 0 && (
                <div>
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider mb-1">Linked Contracts</div>
                  <div className="space-y-1">
                    {selectedRecord.linkedContractIds.map((cid) => (
                      <Link key={cid} href={`/contracts/${cid}`} className="block font-mono text-[12px] text-primary hover:underline flex items-center gap-1">
                        {cid}
              ...
              <div>
                <div className="text-[11px] text-muted-foreground uppercase tracking-wider mb-1">Legal Notes</div>
                <textarea
                  value={legalNotesDraft}
                  onChange={(e) => setLegalNotesDraft(e.target.value)}
                  placeholder="Add review notes for counsel..."
                  className="w-full min-h-[80px] rounded-md border border-border/60 px-2 py-1.5 text-[12px] bg-background placeholder:text-muted-foreground resize-y"
                />
                <Button variant="outline" size="sm" className="h-7 text-[11px] mt-1" onClick={saveLegalNotes}>
                  Save notes
                </Button>
              </div>
